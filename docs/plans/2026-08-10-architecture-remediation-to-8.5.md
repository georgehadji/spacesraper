...
**Status:** In progress — W0, W1, W2, W3, W4 (all of it) complete. W5, W6, W7, W8 not started.
...
| C8 | All five repositories hardcode `aiosqlite` and ignore `settings.database.url`. `docker-compose.enterprise.yml` sets `DB_URL` for a Postgres the app never reads | HIGH | `job_repository.py:62` and four siblings | Open — W5.3 |
...
| C12 | `spacescraper_intel.db` and `spacescraper_jobs.db` hold overlapping "extracted entity lifecycle" concerns | MEDIUM | `sqlite_tracker.py:24` vs `job_repository.py:62` | Open — W5.1 |
...
- [ ] **W5.1** (C12) Collapse `spacescraper_intel.db` into `spacescraper_jobs.db`. Falls out of W2.3
- [ ] **W5.2** (F14) Introduce Unit of Work spanning repositories — shared with W3.6
- [ ] **W5.3** (C8) Decide Postgres. Either implement `PostgresJobRepository` and siblings against the existing `DatabaseSettings` / `database_models.py` and honor `DB_URL`, or delete `DatabaseSettings`, `database_models.py`, `asyncpg`, and `sqlalchemy[asyncio]` and stop advertising Postgres in the enterprise manifest. **Recommendation: implement.** Five separate SQLite connections to one file is already the concurrency ceiling
- [ ] **W5.4** Record the Postgres decision as an ADR
- [ ] **W5.5** If implementing: migration path from the existing SQLite files, reusing `migrate_sqlite_to_postgres.py`

**Exit:** C8, C12 closed. One store, one transaction boundary.
...
- [ ] **W6.1** (C9) Delete `src/saga_orchestrator.py` — unless saga is a committed roadmap item
- [ ] **W6.2** (C9) Delete `src/event_bus.py` — same condition
- [ ] **W6.3** (C10) Delete `src/observability_tracing.py` — unless W7.3 implements tracing for real
- [ ] **W6.4** (C9) Delete `KafkaSettings` from `config_settings.py`, the `aiokafka` dependency, and the `kafka` / `zookeeper` / `kafka-ui` services from `docker-compose.enterprise.yml`
- [ ] **W6.5** Delete the `openai` dependency — unless W7.1 lands multi-provider
- [ ] **W6.6** Delete the `apscheduler` dependency (zero live importers)
- [ ] **W6.7** (C11) Delete the `scheduler` service from `docker-compose.enterprise.yml`, or write the missing `scheduler.py`
- [ ] **W6.8** (C10) Delete the OTel dependencies and the `jaeger` service — unless W7.3 implements tracing
- [ ] **W6.9** Regenerate `docker-compose.enterprise.yml` from what the code actually consumes
- [ ] **W6.10** (G6) Add a CI smoke test that boots the enterprise manifest and hits `/health`, so a manifest referencing a missing entrypoint can never merge again

**Exit:** C9, C10, C11 closed. Manifests match code (satisfies G6).
